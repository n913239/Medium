...
`allowed-tools` 的典型用法是限制 Skill 只能讀取、不能修改：

```yaml
allowed-tools: [Read, Grep, Glob]  # 唯讀 Skill，防止誤改檔案
```
...
這個範例另一個值得注意的地方：加了 `context: fork`。PR diff 可能很長，讓 Skill 在獨立子 agent 裡執行，輸出就不會塞進目前的對話 context。

**原則：輸出量大、或不想讓執行結果影響後續對話的 Skill，就加 `context: fork`。**
...
**Hooks 是事件觸發，Skills 是指令觸發。**

- **Hooks**：每次特定事件發生時自動執行（如：每次 Claude 呼叫某個工具前後自動做某件事）
- **Skills**：你明確呼叫時才執行，或 Claude 依情境判斷後觸發

用一個口訣：
- 「每次都要做的事」→ Hooks
- 「需要時才做的事」→ Skills

兩者可以互補：用 Skill 做主要工作，搭配 Hook 在背景自動做驗證或通知。

*(在這裡插入圖片：skills-vs-hooks.png)*
...
- **真的只做一次** — 直接告訴 Claude 就好，建 Skill 反而多此一舉
- **流程變動很頻繁** — 每次都要修改 SKILL.md，維護成本可能高過直接說
- **已有更好的工具** — CI/CD 腳本、Makefile 更適合的事，不要為了用 Skill 而用

判斷原則：**第二次重複考慮，第三次重複就包起來。**
...
1. **Description 要寫好** — Claude 靠 description 判斷何時自動載入，要描述「使用情境」，不只是功能：
   - ❌ `description: convert markdown`
   - ✅ `description: 將 Markdown 轉為 Word 文件，修正表格框線，適合交付給客戶`

   好的 description 跟用戶說的話對齊，Claude 才能在對的時機自動觸發
...
7. **用對 effort 省推理資源** — `effort: low` 適合格式轉換、程式碼清理這類不需要深度推理的任務；`effort: high` 留給複雜分析或架構決策的場景
