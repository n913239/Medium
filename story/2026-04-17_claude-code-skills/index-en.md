...
A typical use of `allowed-tools` is to make a Skill read-only, preventing accidental file modifications:

```yaml
allowed-tools: [Read, Grep, Glob]  # read-only Skill, no file writes
```
...
One more thing worth noticing in this example: `context: fork`. A PR diff can be long. Running the Skill in an isolated subagent keeps that large output out of your current conversation context.

**Rule of thumb: If a Skill produces a lot of output, or you don't want its results affecting your ongoing conversation, add `context: fork`.**
...
A question that often comes up: how are Skills different from Hooks?

**Hooks are event-driven. Skills are command-driven.**

- **Hooks**: Run automatically when a specific event occurs (e.g., before or after Claude calls a tool)
- **Skills**: Run when you explicitly call them, or when Claude judges the context calls for it

A simple heuristic:
- "Something that should happen every time" → Hooks
- "Something I want to trigger on demand" → Skills
...
Not every repetitive task is worth packaging into a Skill:

- **Truly one-off tasks** — just tell Claude directly; creating a Skill adds unnecessary overhead
- **Frequently changing flows** — if you'd be updating SKILL.md constantly, the maintenance cost may outweigh the benefit
- **Things better suited to other tools** — CI/CD scripts, Makefiles, and git hooks exist for a reason; don't use a Skill just because you can

A practical rule: **consider it the second time, package it the third.**
...
1. **Write a good description** — Claude uses the description to decide when to auto-load. Describe the *use case*, not just what it does:
   - ❌ `description: convert markdown`
   - ✅ `description: Convert Markdown to Word for client delivery, with table border fixes`

   Match the language users would naturally say — that's how Claude knows when to trigger it
...
7. **Use effort to control reasoning depth** — `effort: low` works well for format conversion or code cleanup that needs no deep reasoning; reserve `effort: high` for complex analysis or architectural decisions
