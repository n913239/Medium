...
<tr><td><code>model</code></td><td>指定執行時使用的模型（如 Haiku），可節省 token</td><td>繼承 session</td></tr>
<tr><td><code>effort</code></td><td>推理力度：<code>low</code>、<code>medium</code>、<code>high</code>、<code>max</code></td><td>繼承 session</td></tr>
